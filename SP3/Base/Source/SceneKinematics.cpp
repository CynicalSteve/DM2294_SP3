...
void SceneKinematics::Update(double dt)
{
	...
	if(Application::IsKeyPressed('9') && m_speed < 5.f)
	{
		//Exercise 6: adjust simulation speed
		m_speed += 0.01;
	}
	if(Application::IsKeyPressed('0') && m_speed > Math::EPSILON)
	{
		//Exercise 6: adjust simulation speed
		m_speed -= 0.01;
	...
	if(Application::IsKeyPressed('V'))
	{
		...
		for (std::vector<GameObject *>::iterator it = m_goList.begin(); it != m_goList.end(); ++it)
		{
			...
			if (!go->active)
			{
				...
				go->pos = Vector3(Math::RandFloatMinMax(0, m_worldWidth), Math::RandFloatMinMax(0, m_worldHeight), 0);
				go->vel = Vector3(0, 0, 0);
				go->scale = Vector3(2.f, 2.f, 2.f);
				break;
			}
		}
	}
	...
	if (m_ghost->active)
		m_ghost->vel = m_ghost->pos - Vector3(mouse.x * m_worldWidth / Application::GetWindowWidth(), m_worldHeight - mouse.y * m_worldHeight / Application::GetWindowHeight(), 0);
	if(!bLButtonState && Application::IsMousePressed(0))
	{
		...
		for (std::vector<GameObject *>::iterator it = m_goList.begin(); it != m_goList.end(); ++it)
		{
			...
			if (!go->active)
			{
				...
				m_ghost->pos = Vector3(mouse.x * m_worldWidth / Application::GetWindowWidth(), m_worldHeight - mouse.y * m_worldHeight / Application::GetWindowHeight(), 0);
			}
		}
	}
	else if(bLButtonState && !Application::IsMousePressed(0))
	{
		...
		//v * v = u * u + 2 * a * s
		//s = - (u * u) / (2 * a)
		float maxBallHeight = -m_timeGO->vel.y * m_timeGO->vel.y / (2 * m_gravity.y);
						
		//s = u * t + 0.5 * a * t * t
		//(0.5 * a) * t * t + (u) * t + (-s) = 0
		//
		//m_timeEstimated2 = m_timeEstimated1 + 2 * m_timeGO->pos.y / m_timeGO->vel.y;
	}
	...
	if(!bRButtonState && Application::IsMousePressed(1))
	{
		...
		for (std::vector<GameObject *>::iterator it = m_goList.begin(); it != m_goList.end(); ++it)
		{
			...
			if (!go->active)
			{
				go->active = true;
				go->type = GameObject::GO_CUBE;
				go->pos = Vector3(mouse.x * m_worldWidth / Application::GetWindowWidth(), m_worldHeight - mouse.y * m_worldHeight / Application::GetWindowHeight(), 0);
				go->vel = Vector3(0, 0, 0);
				go->scale = Vector3(2.f, 2.f, 2.f);
				*m_timeGO = *go;
				break;
		...
		if(go->active)
		{
			if(go->type == GameObject::GO_BALL)
			{
				...
				for (std::vector<GameObject *>::iterator it2 = m_goList.begin(); it2 != m_goList.end(); ++it2)
				{
					...
					if (go->type == GameObject::GO_BALL && go2->type == GameObject::GO_CUBE && go2->active)
					{
			...
			if (go->pos.x < 0 || go->pos.x > m_worldWidth || go->pos.y < 0)
				go->active = false;
		}
	}
}
...
void SceneKinematics::Render()
{
	...
	{
		ss.str("");
		ss << "pos: " << m_ghost->pos << " vel: " << m_ghost->vel;
		modelStack.PushMatrix();
		modelStack.Translate(m_ghost->pos.x - m_ghost->vel.x, m_ghost->pos.y - m_ghost->vel.y, 0);
